...
import { cachedCount } from "./lightweight-count"
...
export function accountFromRow(row: Row): AccountRecord {
  return {
    ...
    useBalance: row.use_balance === null ? null : Boolean(row.use_balance), useChinaProviders: Boolean(row.use_china_providers), allowTraining: Boolean(row.allow_training), credentialVersion: Number(row.credential_version),
...
export class AccountRepository {
    ...
    this.db.transaction(() => {
      if (existing) {
        this.db.prepare(`UPDATE accounts SET name=COALESCE(?,name), email=?, auth_cookie_ciphertext=?, go_api_key_ciphertext=?, go_key_id=?,
          ...
          use_china_providers=?, allow_training=?,
          ...
          Number(input.allowTraining ?? false),
        ...
        this.db.prepare(`INSERT INTO accounts(id,owner_user_id,name,workspace_id,email,go_key_id,credential_source,extension_version,last_synced_at,
          ...
          has_manage_subscription_button,billing_guard,use_balance,use_china_providers,allow_training,last_usage_check_at,next_usage_check_at,
          ...
          input.subscriptionState, input.goSubscriptionId ?? null, Number(input.isZenSubscribed ?? false), input.zenSubscriptionId ?? null,
          Number(input.hasManageSubscriptionButton ?? false), input.billingGuard, input.useBalance === null ? null : Number(input.useBalance), Number(input.useChinaProviders ?? false), Number(input.allowTraining ?? false), input.usage ? timestamp : null,
  ...
  updateState(accountId: string, input: Partial<{ name: string; adminState: AdminState; authState: AuthState; subscriptionState: SubscriptionState; goSubscriptionId: string | null; isZenSubscribed: boolean; zenSubscriptionId: string | null; hasManageSubscriptionButton: boolean; billingGuard: BillingGuard; useBalance: boolean | null; useChinaProviders: boolean; allowTraining: boolean; maxConcurrency: number; lastSyncedAt: string; disabledReason: string | null; disabledAt: string | null; lastError: string | null }>): AccountRecord | null {
    ...
    if (input.allowTraining !== undefined) entries.push(["allow_training", Number(input.allowTraining)])
...
export class ApiKeyRepository {
  ...
  // better-sqlite3 同步驱动：LEFT JOIN gateway_requests 实时计数会随日志表增长全扫卡死事件循环。
  // 改为密钥列表（小表）+ 封顶分组扫描（LIMIT 截断，近似值）+ 60s TTL 缓存，耗时恒定。
  listWithCounts():ApiKeyRecord[]{const keys=this.list();const counts=cachedCount(`api-key-request-counts:${this.ownerUserId}`,()=>{const rows=this.db.prepare("SELECT api_key_id,COUNT(*) AS c FROM (SELECT api_key_id FROM gateway_requests WHERE owner_user_id=? AND api_key_id IS NOT NULL LIMIT 20001) GROUP BY api_key_id").all(this.ownerUserId) as Row[];return new Map(rows.map((row)=>[String(row.api_key_id),Number(row.c)]))});return keys.map((key)=>({...key,requestCount:counts.get(key.id)??0}))}
