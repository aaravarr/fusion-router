...
export const POOL_TYPE_METADATA: Partial<Record<PoolType, PoolTypeMeta>> = {
  ...
  "command-code": {
    type: "command-code",
    label: "Command Code",
    description: "Command Code GOAT 套餐（Studio 创建的 user_ API key，长期有效）。Upstream: api.commandcode.ai/provider/v1。chat 收全量 OpenAI/OSS 模型、messages 仅 Claude 系（/responses 不存在，走转换链转 chat）。Quota: alpha/billing/credits 三窗（5h + weekly 双窗 + MONTHLY 余额窗，cap=套餐表）+ subscriptions 套餐/账期；summary 仅对账。窗口调度靠被动错误分类",
    quotaKinds: ["FIVE_HOUR", "WEEKLY", "MONTHLY"] as readonly QuotaKind[],
    credentialFields: [
      { key: "apiKey", label: "API Key", required: true, type: "password" },
    ],
