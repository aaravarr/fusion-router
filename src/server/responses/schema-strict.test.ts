...
describe("schema-strict", () => {
  it("chat/responses tools 的封闭记录递归补 additionalProperties:false", () => {
    ...
    expect((properties.nested as Record<string, unknown>).additionalProperties).toBe(false)
    expect((((properties.list as Record<string, unknown>).items) as Record<string, unknown>).additionalProperties).toBe(false)
    ...
    expect((branchProperties.a as Record<string, unknown>).additionalProperties).toBe(false)
    expect(branches[1].additionalProperties).toBe(false)
    expect((tools[1].parameters as Record<string, unknown>).additionalProperties).toBe(false)
  })
