...
/**
 * Reorder responses input items so every function_call is immediately followed
 * by its function_call_output.
 *
 * Some clients (e.g. Codex) emit assistant message items between a function_call
 * and its output (function_call, message, function_call_output). Strict upstreams
 * (OpenAI-compatible responses interfaces) reject that ordering with
 * "No tool output found for tool call ...". This reorder keeps the call/output
 * pair adjacent and moves any interleaved items (mostly assistant text) after
 * the output. Unpaired trailing function_calls are dropped.
 */
export function reorderResponsesInputItems(body: unknown): { body: unknown; modified: boolean } {
  if (!isObj(body)) return { body, modified: false }
  const input = body.input
  if (!Array.isArray(input) || input.length < 2) return { body, modified: false }

  const isFunctionCall = (item: unknown): item is Obj =>
    isObj(item) && ["function_call", "custom_tool_call", "tool_search_call"].includes(String(item.type || "").toLowerCase())
  const isFunctionCallOutput = (item: unknown): item is Obj =>
    isObj(item) && ["function_call_output", "custom_tool_call_output", "tool_search_output"].includes(String(item.type || "").toLowerCase())
  ...
  for (const item of input) {
    ...
    if (isFunctionCallOutput(item)) {
      ...
      const idx = pending.findIndex((c) => String(c.call_id ?? c.id ?? "") === callId)
      if (idx >= 0) {
        for (let i = 0; i <= idx; i++) output.push(pending[i])
  ...
  if (delayed.length > 0) output.push(...delayed)

  const sig = (items: unknown[]): string =>
    JSON.stringify(items.map((i) => (isObj(i) ? String(i.type || i.role || "item") : "item")))
  if (sig(input) === sig(output) && pending.length === 0) {
    return { body, modified: false }
  }
  return { body: { ...body, input: output }, modified: true }
}
