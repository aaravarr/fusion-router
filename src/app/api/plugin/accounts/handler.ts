...
import { type ApiKeyRecord } from "@/server/repository"
import { AccountOwnershipConflictError } from "@/server/repository"
import { OpenCodeWebError } from "@/server/opencode-web/client"
import { type ReportBrowserAccountInput } from "@/server/opencode-web/service"
...
export const pluginCors = { "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Headers": "authorization,x-api-key,content-type", "Access-Control-Allow-Methods": "POST,OPTIONS" }

type PluginAccountDependencies = {
...
export function createPluginAccountPost(dependencies: PluginAccountDependencies) {
  return async (request: Request) => {
    ...
    if (!apiKey) return Response.json({ error: { type: "authentication_error", message: "Invalid API key" } }, { status: 401, headers: pluginCors })
    const input = schema.safeParse(await request.json().catch(() => null))
    if (!input.success) return Response.json({ error: { type: "validation_error", details: input.error.flatten() } }, { status: 400, headers: pluginCors })
    try {
      ...
      return Response.json({ account, message: "账号已连接，OpenCode Go Key 与额度已同步。" }, { status: 201, headers: pluginCors })
    } catch (cause) {
      ...
      return Response.json({ error: { type: errorType, message } }, { status, headers: pluginCors })
    }
  }
}
