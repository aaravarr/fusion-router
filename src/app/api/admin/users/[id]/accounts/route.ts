...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const poolLabels = listPoolTypeLabelMap(id, db);
  ...
  return Response.json({ accounts: accounts.map((account) => {
    ...
    return { ...account, poolLabel: provider?.displayName ?? poolLabels.get(account.poolType) ?? null, ...deriveAccountRouteStatus(account, accountWindows, Date.now(), { providerReady, providerUnavailableReason }), quotaWindows: accountWindows };
  }) });
}
